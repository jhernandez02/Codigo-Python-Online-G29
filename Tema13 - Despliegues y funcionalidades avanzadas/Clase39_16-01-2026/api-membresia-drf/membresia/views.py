...
def generar_boleta_electronica(boletaData):
    item = {
        ...
        "tipo_de_igv": 1,
        "igv": boletaData['item_precio']*0.18,
    ...
    suscripcion = Suscripcion.objects.get(pk=boletaData['nro_correlativo'])
    suscripcion.enlace_boleta = jsonDict['enlace_del_pdf']
    suscripcion.save()

def procesar_pago(suscripcion):
    boletaData = {
        "nro_correlativo": suscripcion.id,
...
class SuscripcionUsuarioListView(generics.ListAPIView):
    serializer_class = SuscripcionSerializer
